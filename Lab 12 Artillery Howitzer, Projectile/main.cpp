...
void callBack(const Interface* pUI, void* p)
{
   ...
   bool isFlying = true;
   if (pSim->projectile.getPosition().getMetersY()
       >= pSim->ground.getElevationMeters(pSim->projectile.getPosition()))
   {
   ...
   if (!isFlying)
   {
      ...
      if (pSim->projectile.getPosition().getMetersX()
            >= (pSim->ground.getTarget().getMetersX() - 175) &&
          pSim->projectile.getPosition().getMetersX()
            <= (pSim->ground.getTarget().getMetersX() + 175))
      {
   ...
   for (int i = 1; i < 20; i++)
   {
      double diffX = pSim->projectilePath[i].getPixelsX()
                   - bulletPos.getPixelsX();
      double diffY = pSim->projectilePath[i].getPixelsY()
                   - bulletPos.getPixelsY();
